...
class NorthAmerica extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            country: props.match.params.country ? props.match.params.country.toUpperCase() : 'US',
            games: [],
            discounts: [],
            done: false,
            baseUri,
    ...
    getAmericaPrices(){
        ...
        gameIds.forEach((chunk) => {
            ...
            allAxios.push(
                axios.get(`${this.state.baseUri}getPrices/${this.state.country}/${chunk.join(",")}/${offset}`, {cancelToken: this.state.axiosSource.token})
                    .then(response => {
                        // handle success
                        // console.log("getAmericaPrices ", response.data);
    ...
    getAmericaGames() {
        ...
        let offset = 0;
        axios.get(`${this.state.baseUri}getAmericaGames/${limit}/${offset}`, {cancelToken: this.state.axiosSource.token})
            .then(response => {
                // handle success
                console.log("getAmericaGames ",response);

                let games = {games: response.data};
                this.setState({...games});

                this.getAmericaPrices();
            })
            .catch(function (error) {
                // handle error
                console.log(error);
    ...
    componentDidMount() {
        this.getAmericaGames();
        document.title = "Nintendo Deals - North America"
    }
                ...
                <div className="row">
                    <div className="col">
                        <h1 className={"display-3"}>
                            {this.state.country === 'US' && "United States"}
                            {this.state.country === 'CA' && "Canada"}
                            {this.state.country === 'MX' && "Mexico"}
                            {this.state.country !== 'MX' && this.state.country !== 'US' && this.state.country !== 'CA' && this.state.country}
                        </h1>
                                    ...
                                    return (
                                        <tr key={game.nsuid}>
                                            <td className={"p-1 align-middle"}>
                                                <div className={"-rounded-circle"} style={{width: "65px", height: "65px", backgroundImage: `url(${game.front_box_art})`, backgroundSize: "75px", backgroundPosition: "center", backgroundRepeat: "no-repeat"}}/>
                                            </td>
